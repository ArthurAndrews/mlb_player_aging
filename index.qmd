...
hit |> 
  ...
  pivot_longer(-age) |> 
  ggplot() + aes(age, value, color = name) + geom_line() + labs(title = "splines")
```
...
```{r}
model2 <- lmer(
  ops ~ (1 | name) + bs(centered_age, df = df),
  data = hit
)
...
mean_player <- tibble(age = age_range + mean(hit$age), centered_age = age_range) |> 
  add_prediction(model2, re.form = ~0)

mean_player |> 
  ggplot() + aes(age, pred_ops) + geom_line(color = pal[1]) + labs(title = "modeled aging curve", y = "ops") +
